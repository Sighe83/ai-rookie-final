...
async function createTestLearnerUser() {
  ...
  const testLearnerEmail = 'learner.test@airookie.com'
  const testLearnerPassword = 'LearnerTest123!'
  const testLearnerName = 'Test Learner'

  try {
    console.log('Creating test learner user...')

    // Step 1: Create user in Supabase Auth with verified email
    const { data: authUser, error: authError } = await supabaseAdmin.auth.admin.createUser({
      email: testLearnerEmail,
      password: testLearnerPassword,
      email_confirm: true, // This sets the email as verified
      user_metadata: {
        name: testLearnerName,
        role: 'learner'
      }
    })
    ...
    if (authError) {
      ...
      if (authError.message.includes('already exists')) {
        ...
        const { data: { users }, error: listError } = await supabaseAdmin.auth.admin.listUsers({
          filter: `email.eq.${testLearnerEmail}`
        })
        ...
        if (existingDbUser) {
          ...
          console.log('\n✅ Test Learner User Already Exists:')
          console.log('Email:', testLearnerEmail)
          console.log('Password:', testLearnerPassword)
        ...
        const dbUser = await prisma.user.create({
          data: {
            ...
            name: testLearnerName,
            role: 'learner',
            bio: 'Enthusiastic learner ready to master AI tools and automation',
    ...
    console.log('\n✅ Test Learner User Created Successfully!')
    console.log('=====================================')
    console.log('Email:', testLearnerEmail)
    console.log('Password:', testLearnerPassword)
    ...
    console.error('Error creating test learner user:', error)
    throw error
  } finally {
