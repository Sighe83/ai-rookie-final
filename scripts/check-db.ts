...
async function main() {
  ...
  try {
    ...
    for (const expectedFK of expectedSetNullFKs) {
      const matchingConstraint = fkConstraints.find(
        (fk) =>
      ...
      if (!matchingConstraint) {
        console.error(
          `❌ Foreign key constraint not found: ${expectedFK.source}.${expectedFK.column} -> ${expectedFK.target}`
        )
        process.exit(1)
      }
      ...
      if (matchingConstraint.confdeltype !== 'n') {
        ...
        console.error(
          `❌ Wrong DELETE action for ${expectedFK.source}.${expectedFK.column} -> ${expectedFK.target}`
        )
        console.error(
          `   Expected: SET NULL, Got: ${actionMap[matchingConstraint.confdeltype] || matchingConstraint.confdeltype}`
        )
        process.exit(1)
      }
    }
    ...
    console.log(
      '✅ All required foreign key constraints have correct ON DELETE SET NULL'
    )
    ...
    if (Number(bookingCount[0].count) === 0) {
      ...
      if (denormMismatches.length > 0) {
        console.error(
          `❌ Denormalization mismatch found in Booking.expertUserId:`
        )
        for (const mismatch of denormMismatches) {
          console.error(
            `   Booking ${mismatch.booking_id}: expertUserId="${mismatch.expert_user_id}" != ExpertProfile.userId="${mismatch.profile_user_id}"`
          )
        }
        process.exit(1)
      }
